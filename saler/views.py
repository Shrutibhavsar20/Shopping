...
def saler_reg(request):
    ...
    return render(request,'saler/saler_reg.html',context)
...
def store_reg(request):
    ...
    if mypassword == mycpassword and check_username(request,myusername) and check_email(request,myemail):
        ...
        if check_username(request,myusername) is False:
            ...
            return redirect('/saler/saler_reg')
...
def edit_reg(request,id):
    ...
    return render(request,'saler/edit_reg.html',context)
...
def store_product(request):
    ...
    Product.objects.create(categories_id=mycid,subcategories_id=myscid,product_name=mypname,image=myimage,price=myprice,details=mydetails,quantity=myquantity,saler_reg_id=saler.id)
...
def orders(request):
    user_id = request.user.id
    saler_id = Saler_reg.objects.get(user_id=user_id)
    orders = Order.objects.filter(saler_id=saler_id)
    context = {'orders':orders}
    return render(request,'saler/orders.html',context)
...
def order_details(request,id):
    ...
    return render(request,'saler/order_details.html',context)
...
def search_orders(request):
    ...
    return render(request, 'saler/orders.html', {'orders': orders, 'start_date': start_date, 'end_date': end_date})
...
def generate_pdf_order(request):
    # Get the start and end dates from the request
    start_date = request.GET.get('start_date')
    end_date = request.GET.get('end_date')
    result = []
